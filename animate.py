...
def create_animation(
    ...
    *,
    ...
    Args:
        ...
        vmin: Minimum values for color scaling, either a single value or a list of values for each subplot.
        vmax: Maximum values for color scaling, either a single value or a list of values for each subplot.
        cmap: Colormap for the plots, either a single value or a list of values for each subplot.
    ...
    with h5_context as h5file:
        with mpl_context:
            ...
            collections = []
            for i, (quantity, ax) in enumerate(zip(quantities, axes.flat)):
                ax: plt.Axes
                ...
                collection = ax.tripcolor(
                    ...
                    shading=shading,
                    cmap=cmap[i] if cmap[i] is not None else opts.cmap,
                    vmin=vmin[i] if vmin[i] is not None else opts.vmin,
                    vmax=vmax[i] if vmax[i] is not None else opts.vmax,
                )
            ...
            def update(frame):
                ...
                for i, (quantity, collection) in enumerate(
                    ...
                    if opts.vmin is None and vmin[i] is None:
                        if autoscale:
                            vmins[i] = np.min(values[mask])
                        else:
                            vmins[i] = min(vmins[i], np.min(values[mask]))
                    else:
                        vmins[i] = vmin[i] if vmin[i] is not None else opts.vmin
                    if opts.vmax is None and vmax[i] is None:
                        if autoscale:
                            vmaxs[i] = np.max(values[mask])
                        else:
                            vmaxs[i] = max(vmaxs[i], np.max(values[mask]))
                    else:
                        vmaxs[i] = vmax[i] if vmax[i] is not None else opts.vmax
                    if opts.symmetric:
                        vmax_val = max(abs(vmins[i]), abs(vmaxs[i]))
                        vmaxs[i] = vmax_val
                        vmins[i] = -vmax_val
                    if shading == "flat":
